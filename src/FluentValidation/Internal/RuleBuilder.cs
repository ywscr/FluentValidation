...
namespace FluentValidation.Internal {
	...
	public class RuleBuilder<T, TProperty> : IRuleBuilderOptions<T, TProperty>, IRuleBuilderInitial<T, TProperty>, IRuleBuilderInitialCollection<T, TProperty> {
		/// <summary>
		...
		public IRuleBuilderOptions<T, TProperty, TValidator> SetValidator<TValidator>(TValidator validator) where TValidator : IPropertyValidator {
			...
			Rule.AddValidator(validator);
			return new ValidatorOptionsBuilder<TValidator>(Rule, ParentValidator, validator);
		}
		...
		public IRuleBuilderInitialCollection<T, TProperty> Configure(Action<CollectionPropertyRule<T, TProperty>> configurator) {
		...
		private class ValidatorOptionsBuilder<TValidator> : RuleBuilder<T, TProperty>, IRuleBuilderOptions<T, TProperty, TValidator> where TValidator : IPropertyValidator {
			private readonly TValidator _propertyValidator;

			public ValidatorOptionsBuilder(PropertyRule<T> rule, AbstractValidator<T> parent, TValidator propertyValidator)
				: base(rule, parent) {
				_propertyValidator = propertyValidator;
			}

			public IRuleBuilderOptions<T, TProperty, TValidator> Configure(Action<PropertyRule<T>, TValidator> configurator) {
				configurator(Rule, _propertyValidator);
				return this;
			}

			public IRuleBuilderOptions<T, TProperty, TValidator> Configure(Action<PropertyRule<T>> configurator) {
				configurator(Rule);
				return this;
			}
